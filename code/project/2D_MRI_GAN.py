...
class MRIGANDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        age = int(sample["age"])
        age_vector = torch.zeros(20, dtype=torch.float32)  # 20-dimensional vector
        if age > 0:
...
def train_gan(
    ...
    # SIMPLER BALANCE: Focus on discriminator balance and basic content preservation
    adversarial_weight = getattr(hparams, "adversarial_weight", 1.0)  # Back to original
    ...
    d_train_freq = getattr(
        hparams, "d_train_freq", 1
    )  # Train D every iteration (increased frequency)
    ...
    if use_wgan_gp:
        print(f"Using WGAN-GP with gradient penalty weight: {gradient_penalty_weight}")
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (real_imgs, age_vecs, asd_vecs) in enumerate(train_loader):
            ...
            real_age_vecs = age_vecs.to(
                device, non_blocking=True
            )  # Real ages for discriminator
            ...
            if getattr(hparams, "enable_age_manipulation", True):
                ...
                for i in range(batch_size):
                    if manipulation_mask[i]:
                        ...
                        age_shift = torch.randint(
                            1, 4, (1,)
                        ).item()  # Generate integers 1, 2, 3
                        new_age = max(1, min(20, current_age + age_shift))
            ...
            if should_train_discriminator:
                ...
                with torch.cuda.amp.autocast():
                    ...
                    fake_output = discriminator(fake_imgs_noisy, new_age_vecs, asd_vecs)
                    ...
                    if use_wgan_gp:
                        ...
                        d_loss = (
                            d_loss_real + d_loss_fake + gradient_penalty_weight * gp
                        )
                    else:
                ...
                with torch.no_grad():
                    ...
                    if use_wgan_gp:
                        d_loss_real = -torch.mean(real_output)
                        d_loss_fake = torch.mean(fake_output)
                        current_d_loss = (d_loss_real + d_loss_fake).item()
                    else:
                        d_loss_real = adversarial_loss(real_output, real_labels)
                        d_loss_fake = adversarial_loss(fake_output, fake_labels)
            ...
            if batch_idx % g_train_freq == 0:
                ...
                with torch.cuda.amp.autocast():
                    ...
                    if use_wgan_gp:
                        g_loss_adv = -torch.mean(fake_output_gen)
                    else:
...
def main(hparams):
    ...
    cached_dataset = create_monai_dataset(
    ...
    batch_size = getattr(hparams, "batch_size", 16)  # Reduced default from 32 to 16
